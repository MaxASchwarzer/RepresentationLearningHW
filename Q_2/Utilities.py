...
class mnistDataLoader() :
	...
	def __init__(self, batch_size = 256) :
		...
		with open("mnist.pkl",'rb') as f:
			mnist = pickle.load(f)
		self.x_train = mnist["training_images"].astype(np.float32)*1.0/255.0
		self.y_train = mnist["training_labels"].astype(np.float32)
		self.x_validation = mnist["test_images"].astype(np.float32)*1.0/255.0
		self.y_validation = mnist["test_labels"].astype(np.float32)
...
def GetPointwiseGradientOfNonLinearity(feat, non_linearity) :
	...
	if non_linearity == 'ReLU' :
		...
		return Sigmoid(feat) * (1.0 - Sigmoid(feat) )
	elif non_linearity == 'Tanh' :
		return 1.0 - np.power(np.tanh(feat), 2)
	elif non_linearity == 'Linear' : # Solely for input!!
		return np.ones_like(feat)
	else :
...
if __name__ == '__main__' :
	...
	print('[DEBUG] Batch Max Value : ', np.max(mnist.x_train), ' Batch Min Value : ', np.min(mnist.x_train))

	print('##################################################')
	print('########## TEST : Standard MNIST Download')
	print('##################################################')
	# DownloadStandardMNIST()
